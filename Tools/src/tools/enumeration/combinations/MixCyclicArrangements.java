...
public class MixCyclicArrangements<A> implements Iterable<List<A>> {
	...
	public MixCyclicArrangements(List<A> list, int c) {
		l = list;
		n = list.size();
		this.c = c;
		max = f(n)/(f(c)*f(n-c));
	...
	public Iterator<List<A>> iterator() {
		return new Iterator<List<A>>() {
			...
			public List<A> next() {
				...
				subIterator = new MixCyclicPermutations<A>(list).iterator();
				id++;
				provided++;
