...
public class ScanLl {
	...
	public static Ll readBinaryLine() { return Arrays.stream(Scan.readLine().strip().split("\\s+")).map(s -> Long.parseLong(s, 2)).collect(Collectors.toCollection(Ll::new)); }
	public static Ll readHexLine() { return Arrays.stream(Scan.readLine().strip().split("\\s+")).map(s -> Long.parseLong(s, 16)).collect(Collectors.toCollection(Ll::new)); }
